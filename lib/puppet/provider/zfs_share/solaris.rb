Puppet::Type.type(:zfs_share).provide(:solaris) do
  ...
  def create
    zfscmd("set", "share=#{@resource[:zfs_name].gsub('/', '_')},path=#{@resource[:zfs_name].insert(0,'/')},prot=nfs,sec=#{security},#{permission}=#{@resource[:allow_ip].insert(0, '@')}", @resource[:zfs_name])
